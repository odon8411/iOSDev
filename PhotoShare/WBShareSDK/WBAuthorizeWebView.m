...
//#define kQQCallback @"open.t.qq.com"
...
- (BOOL)webView:(UIWebView *)aWebView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    if ([snsType isEqualToString:@"sina"]) 
	{
        ...
        if (range.location != NSNotFound)
        {
            ...
            if ([delegate respondsToSelector:@selector(authorizeWebView:didReceiveAuthorizeCode:withQQopenid:)])
            {
                [delegate authorizeWebView:self didReceiveAuthorizeCode:code withQQopenid:nil];
            }
        }
	...
	else if([snsType isEqualToString:@"qq"])
	{
        ...
        if (range.location != NSNotFound)
        {
			...
			NSString *code = [self getStringFromUrl:[request.URL absoluteString] needle:@"code="];
			NSString *openid = [self getStringFromUrl:[request.URL absoluteString] needle:@"openid="];
 
            if ([delegate respondsToSelector:@selector(authorizeWebView:didReceiveAuthorizeCode:withQQopenid:)])
            {
                [delegate authorizeWebView:self didReceiveAuthorizeCode:code withQQopenid:openid];
            }
        }
